...
```{python}
#| label: fig-arena
#| fig-cap: "Every solver in ALL_SOLVERS against Himmelblau, sorted best-to-worst, log-scale. Gradient-based methods (Newton, BFGS, L-BFGS) land near machine precision; particle swarm, a population method, actually lands ahead of all of them yet reports 'max_iter reached' rather than a gradient-based convergence flag -- a different stopping rule, not a worse answer, and still within 1e-5 of the true minimum."
import numpy as np
...
QP Chapter 3 already solves in closed form: minimize a portfolio's variance
...
model, kept a pure equality-constrained QP rather than needing Chapter 5's general
...
#| fig-cap: "Three users, two shared resources (resource 0 shared between users 0,1; resource 1 shared between users 1,2). User 1 contends for both resources at once and is squeezed to about half either edge user's share -- both resources land at exactly their capacity."
...
Solved via Chapter 5's `barrier_method` — proportional fairness is a genuine
...
search spaces than these 2D toys, and separately, a full Gaussian process's `O(n^3)`
cost per fit — which scales with the number of evaluations collected so far, not with
search-space dimensionality — becomes the bottleneck GP-based methods are actually
known for as evaluation budgets grow), just an honest result on the specific
low-dimensional, modest-budget comparison run here.
...
pendulum swing-up — already has its worked example: Chapter 8's
