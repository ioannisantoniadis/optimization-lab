...
machinery. Phase 7 turns outward: inverse problems (image deblurring, nonlinear system
identification), control (LQR, nonlinear optimal control via direct shooting, and
grid-world value iteration), and machine learning end to end — manual backpropagation
and a physics-informed neural network, both trained with this repo's *own* from-scratch
`adam`/`bfgs`, the same solvers that trained every network in this chapter.
